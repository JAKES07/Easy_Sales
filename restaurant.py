...
def kitchen_token(store_id):
    """Return a persistent private Kitchen Screen token for this store.

    The token is stored in the store's own restaurant database instead of
    being derived from EASY_SALES_SECRET_KEY. This is important on hosted
    deployments: changing/reloading an environment secret must not suddenly
    invalidate a Kitchen Screen URL that is already being used on another
    device.
    """
    ...
    c = get_store_connection(sid)
    try:
        c.execute("CREATE TABLE IF NOT EXISTS restaurant_settings (key TEXT PRIMARY KEY, value TEXT)")
        row = c.execute("SELECT value FROM restaurant_settings WHERE key='kitchen_access_token'").fetchone()
        token = str(row[0] or '').strip() if row else ''
        if not token:
            token = secrets.token_urlsafe(32)
            c.execute("INSERT OR REPLACE INTO restaurant_settings(key,value) VALUES('kitchen_access_token',?)", (token,))
            c.commit()
        return token
    finally:
        c.close()
...
def valid_kitchen_token(store_id, token):
    ...
    # Primary authentication: persistent token stored in this store's
    # restaurant database. This survives app restarts and secret-key changes.
    try:
    ...
    # Backward compatibility for Kitchen URLs generated by the previous build.
    # Those URLs continue working as long as their original secret is still in
    # use; newly generated URLs always use the persistent token above.
...
def kitchen_page():
    ...
    # Initialise the store database first so the persistent Kitchen token
    # exists even when the Kitchen device is the first device to open the
    # screen after a deployment/restart.
    init_restaurant_db(store_id)

    # On the owner's device the normal Store Access session is enough.
    # On a second device the persistent private token in the shared URL is used.
